...
local function buf_change_env(client, bufnr)
  local new
  vim.ui.input({ prompt = "New environment name: " }, function(input) new = input end)
  if not new or new == "" then return vim.notify("No environment name provided", vim.log.levels.WARN) end
